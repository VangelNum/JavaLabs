...
public class Population {
    ...
    public void parentPullGen(){
        int parSize = (int)(Math.random()*((0.5*size-0.25*size)+1) + 0.25*size); // устанавливаем размер [12;25]
        this.parents = new Individ[parSize];
        int[] arr = new int[parSize]; // Родительский массив [12;25 Элементов]
        for (int p = 0; p<parSize;p++) {
            double randNum = Math.random() * size - 1;
            parents[p] = population[(int) randNum];
        }


//        for(int p = 0; p < parSize; p++){ //20
//            boolean flag = false;
...
//        }
    ...
    public void crossing(){
        ...
        for(int i = 0; i < size; i++) {
            ...
            int randPar3 = (int)(Math.random()*((pl)));
            ...
            Chromosome ch4 = new Chromosome(parents[randPar2].getWeights());

            ch4.hybritization(ch1.getChrom(), ch2.getChrom(),ch3.getChrom());
            ch3.mutation();
